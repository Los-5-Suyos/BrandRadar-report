...
|---------|--------|-------------|
| EP01 | Gestión de Cuentas | Registro, verificación de email e inicio de sesión para PyME Owner y Agency Manager. |
| EP02 | Configuración de Marca | Creación y configuración de marcas/clientes con palabras clave y fuentes de datos. |
| EP03 | Monitoreo de Menciones | Inicio del monitoreo, recolección y filtrado de menciones desde plataformas digitales. |
| EP04 | Análisis de Sentimiento | Clasificación automática del sentimiento de las menciones y detección de tendencias. |
| EP05 | Gestión de Alertas | Generación, notificación y respuesta a alertas críticas de reputación. |
| EP06 | Reportes y Dashboard | Visualización del dashboard, generación y exportación de reportes de reputación. |
| EP07 | Landing Page | Sitio web estático informativo dirigido a visitantes PyME y de agencia digital. |
| EP08 | RESTful API — Autenticación | Endpoints de registro, verificación y autenticación de usuarios. |
| EP09 | RESTful API — Marcas | Endpoints para creación, configuración y gestión de marcas. |
| EP10 | RESTful API — Menciones | Endpoints para consulta y filtrado de menciones recolectadas. |
| EP11 | RESTful API — Sentimiento | Endpoints para análisis y consulta de sentimiento por marca. |
| EP12 | RESTful API — Alertas | Endpoints para gestión y respuesta de alertas de reputación. |
| EP13 | RESTful API — Reportes | Endpoints para generación y exportación de reportes. |
...
|-----------------|--------|-------------|-------------------------|---------------------------|
| EP01 | Gestión de Cuentas | Agrupa todas las funcionalidades de registro, verificación y acceso al sistema para ambos segmentos. | — | — |
| US01 | Registrar cuenta | Como PyME Owner o Agency Manager, quiero crear una cuenta en BrandRadar con mis datos básicos para acceder a las funcionalidades de monitoreo de reputación. | **Escenario 1 – Registro exitoso:** <br> **Given** que el usuario completa todos los campos obligatorios (nombre, correo, contraseña y tipo de cuenta), <br> **When** envía el formulario de registro, <br> **Then** el sistema crea la cuenta, envía un correo de verificación y muestra el mensaje "Cuenta creada. Revisa tu correo para verificarla."<br><br> **Escenario 2 – Correo ya registrado:** <br> **Given** que el usuario ingresa un correo electrónico que ya existe en el sistema, <br> **When** envía el formulario, <br> **Then** el sistema muestra el mensaje "El correo ya está registrado" y no crea una cuenta duplicada. <br><br> **Escenario 3 – Contraseña débil:** <br> **Given** que el usuario ingresa una contraseña que no cumple los requisitos mínimos de seguridad (mínimo 8 caracteres, una mayúscula y un número), <br> **When** envía el formulario, <br> **Then** el sistema resalta los requisitos incumplidos y no permite continuar el registro. | EP01 |
| US02 | Verificar correo electrónico | Como usuario recién registrado, quiero verificar mi correo electrónico para activar mi cuenta y acceder a BrandRadar. | **Escenario 1 – Verificación exitosa:** <br> **Given** que el usuario recibe el correo de verificación y hace clic en el enlace dentro de las 24 horas, <br> **When** el sistema valida el token, <br> **Then** la cuenta queda activada y el sistema redirige al usuario al login con el mensaje "Correo verificado exitosamente." <br> **Escenario 2 – Token expirado:** <br> **Given** que el usuario hace clic en el enlace de verificación después de 24 horas, <br> **When** el sistema intenta validar el token, <br> **Then** el sistema muestra el mensaje "El enlace ha expirado" y ofrece la opción de reenviar el correo de verificación. <br> **Escenario 3 – Token inválido:** <br> **Given** que el usuario accede a una URL de verificación con un token manipulado o incorrecto, <br> **When** el sistema intenta validarlo, <br> **Then** el sistema muestra el mensaje "Enlace de verificación inválido" y no activa la cuenta. | EP01 |
| US03 | Iniciar sesión | Como PyME Owner o Agency Manager con cuenta verificada, quiero iniciar sesión en BrandRadar para acceder a mi panel de monitoreo. | **Escenario 1 – Login exitoso:** Given que el usuario ingresa credenciales válidas (correo y contraseña) y su cuenta está verificada, When envía el formulario de login, Then el sistema inicia la sesión, genera un token de autenticación y redirige al dashboard principal. **Escenario 2 – Credenciales incorrectas:** Given que el usuario ingresa una contraseña incorrecta, When envía el formulario de login, Then el sistema muestra el mensaje "Correo o contraseña incorrectos" y no otorga acceso. **Escenario 3 – Cuenta no verificada:** Given que el usuario intenta iniciar sesión con una cuenta cuyo correo aún no fue verificado, When envía el formulario, Then el sistema muestra el mensaje "Debes verificar tu correo antes de iniciar sesión" y ofrece reenviar el correo de verificación. | EP01 |
| US04 | Cerrar sesión | Como usuario autenticado, quiero cerrar sesión en BrandRadar para proteger el acceso a mi cuenta desde dispositivos compartidos. | **Escenario 1 – Cierre exitoso:** Given que el usuario está autenticado y hace clic en "Cerrar sesión", When el sistema procesa la solicitud, Then la sesión se invalida, el token de autenticación se elimina y el sistema redirige a la página de login. **Escenario 2 – Sesión ya expirada:** Given que el token de sesión del usuario ya expiró, When el usuario intenta acceder a una ruta protegida, Then el sistema redirige automáticamente al login sin mostrar datos protegidos. **Escenario 3 – Cierre desde múltiples pestañas:** Given que el usuario tiene BrandRadar abierto en dos pestañas y cierra sesión en una de ellas, When intenta navegar en la otra pestaña, Then el sistema detecta la sesión inválida y redirige al login. | EP01 |
| EP02 | Configuración de Marca | Agrupa la creación y configuración de marcas propias (PyME) y marcas de clientes (Agency Manager) con palabras clave y fuentes de datos conectadas. | — | — |
| US05 | Crear perfil de marca (PyME) | Como PyME Owner, quiero crear el perfil de mi marca ingresando información básica y palabras clave para que BrandRadar comience a monitorearla. | **Escenario 1 – Creación exitosa:** Given que el PyME Owner completa el nombre de la marca, descripción y al menos una palabra clave, When guarda la configuración, Then el sistema registra la marca, confirma con el mensaje "Marca creada exitosamente" y la lista en el panel de marcas. **Escenario 2 – Nombre de marca duplicado:** Given que el PyME Owner ingresa un nombre de marca que ya existe en su cuenta, When intenta guardar, Then el sistema muestra el mensaje "Ya tienes una marca con ese nombre" y no crea un duplicado. **Escenario 3 – Sin palabras clave:** Given que el PyME Owner no ingresa ninguna palabra clave al crear la marca, When intenta guardar, Then el sistema muestra el mensaje "Debes agregar al menos una palabra clave para el monitoreo" y no permite continuar. | EP02 |
| US06 | Agregar marca de cliente (Agency Manager) | Como Agency Manager, quiero agregar una nueva marca de cliente a mi panel para comenzar a gestionar su reputación digital. | **Escenario 1 – Alta de cliente exitosa:** Given que el Agency Manager completa el nombre del cliente, sector y al menos una palabra clave, When guarda el perfil, Then el sistema registra la marca cliente, la asocia a su cuenta de agencia y la muestra en la lista de cuentas gestionadas. **Escenario 2 – Límite de cuentas alcanzado:** Given que el Agency Manager ha alcanzado el límite de marcas incluidas en su plan, When intenta agregar una nueva marca, Then el sistema muestra el mensaje "Has alcanzado el límite de marcas de tu plan actual" y ofrece la opción de actualizar el plan. **Escenario 3 – Datos incompletos:** Given que el Agency Manager no completa el campo de nombre del cliente, When intenta guardar, Then el sistema resalta los campos obligatorios vacíos y no registra la marca. | EP02 |
| US07 | Configurar palabras clave de monitoreo | Como PyME Owner o Agency Manager, quiero configurar y editar las palabras clave asociadas a una marca para asegurar que el monitoreo capture las menciones relevantes. | **Escenario 1 – Edición exitosa:** Given que el usuario accede a la configuración de palabras clave de una marca existente y modifica o agrega términos, When guarda los cambios, Then el sistema actualiza las palabras clave y muestra el mensaje "Palabras clave actualizadas correctamente." **Escenario 2 – Palabra clave duplicada:** Given que el usuario intenta agregar una palabra clave que ya existe para esa marca, When confirma la adición, Then el sistema muestra el mensaje "Esta palabra clave ya fue agregada" y no genera duplicados. **Escenario 3 – Límite de palabras clave:** Given que el usuario intenta agregar una palabra clave cuando ya se alcanzó el límite permitido por el plan, When confirma la adición, Then el sistema muestra el mensaje "Has alcanzado el límite de palabras clave para este plan" y no agrega el término. | EP02 |
| US08 | Conectar fuentes de datos | Como PyME Owner o Agency Manager, quiero conectar plataformas digitales (Google Maps, redes sociales) a una marca para que BrandRadar recolecte menciones de esas fuentes. | **Escenario 1 – Conexión exitosa:** Given que el usuario selecciona una plataforma disponible y completa la configuración de conexión, When confirma la integración, Then el sistema valida la conexión, muestra el estado "Conectado" junto a la fuente y la incluye en el monitoreo activo. **Escenario 2 – Credenciales inválidas:** Given que el usuario ingresa credenciales incorrectas para conectar una plataforma, When el sistema intenta validar la conexión, Then el sistema muestra el mensaje "No se pudo conectar. Verifica tus credenciales" y mantiene la fuente como no conectada. **Escenario 3 – Plataforma no disponible:** Given que el usuario intenta conectar una fuente cuyo servicio externo está temporalmente inactivo, When el sistema intenta establecer la conexión, Then el sistema muestra el mensaje "La plataforma no está disponible en este momento. Intenta más tarde." | EP02 |
| EP03 | Monitoreo de Menciones | Agrupa las funcionalidades de inicio del proceso de monitoreo, recolección de menciones y filtrado por criterios relevantes. | — | — |
| US09 | Iniciar monitoreo de marca | Como PyME Owner o Agency Manager, quiero iniciar el monitoreo de una marca configurada para que BrandRadar comience a recolectar menciones en tiempo real. | **Escenario 1 – Inicio exitoso:** Given que la marca tiene al menos una palabra clave y una fuente conectada, When el usuario activa el monitoreo, Then el sistema inicia el proceso, actualiza el estado de la marca a "Monitoreando" y muestra la fecha y hora de inicio. **Escenario 2 – Configuración incompleta:** Given que la marca no tiene fuentes de datos conectadas, When el usuario intenta iniciar el monitoreo, Then el sistema muestra el mensaje "Conecta al menos una fuente de datos antes de iniciar el monitoreo" y no inicia el proceso. **Escenario 3 – Monitoreo ya activo:** Given que el monitoreo de la marca ya está en ejecución, When el usuario intenta iniciarlo nuevamente, Then el sistema muestra el mensaje "El monitoreo de esta marca ya está activo" y no genera un proceso duplicado. | EP03 |
| US10 | Visualizar menciones recolectadas | Como PyME Owner o Agency Manager, quiero ver el listado de menciones recolectadas de mi marca para estar al tanto de lo que se dice en internet. | **Escenario 1 – Listado con menciones:** Given que el sistema ha recolectado menciones para la marca seleccionada, When el usuario accede a la sección de menciones, Then el sistema muestra el listado con fuente, fecha, texto y sentimiento de cada mención. **Escenario 2 – Sin menciones aún:** Given que el monitoreo está activo pero aún no se han recolectado menciones, When el usuario accede a la sección, Then el sistema muestra el mensaje "No se han encontrado menciones aún. El monitoreo está en proceso." **Escenario 3 – Error de conexión con fuente:** Given que una fuente de datos presenta falla en la conexión durante el monitoreo, When el usuario accede al listado, Then el sistema muestra un aviso indicando la fuente con error y continúa mostrando las menciones de las fuentes disponibles. | EP03 |
| US11 | Filtrar menciones por criterio | Como PyME Owner o Agency Manager, quiero filtrar las menciones recolectadas por fecha, fuente o sentimiento para encontrar rápidamente las más relevantes. | **Escenario 1 – Filtro aplicado con resultados:** Given que el usuario aplica uno o más filtros (fecha, fuente o sentimiento) sobre el listado de menciones, When confirma el filtro, Then el sistema actualiza el listado mostrando únicamente las menciones que cumplen los criterios seleccionados. **Escenario 2 – Filtro sin resultados:** Given que el usuario aplica filtros que no coinciden con ninguna mención existente, When confirma el filtro, Then el sistema muestra el mensaje "No hay menciones que coincidan con los filtros aplicados." **Escenario 3 – Resetear filtros:** Given que el usuario tiene filtros activos aplicados al listado, When hace clic en "Limpiar filtros", Then el sistema elimina todos los filtros y muestra el listado completo de menciones. | EP03 |
| EP04 | Análisis de Sentimiento | Agrupa las funcionalidades de clasificación automática del sentimiento de las menciones y visualización de tendencias. | — | — |
| US12 | Ver análisis de sentimiento general | Como PyME Owner o Agency Manager, quiero ver el análisis de sentimiento general de mi marca para entender cómo está siendo percibida en internet. | **Escenario 1 – Análisis disponible:** Given que la marca tiene menciones clasificadas por sentimiento, When el usuario accede a la sección de análisis de sentimiento, Then el sistema muestra la distribución porcentual de menciones positivas, negativas y neutras con un gráfico resumen. **Escenario 2 – Análisis en proceso:** Given que el sistema está procesando menciones recientes y aún no ha completado la clasificación, When el usuario accede a la sección, Then el sistema muestra el estado "Análisis en proceso" junto con los datos ya disponibles. **Escenario 3 – Sin datos suficientes:** Given que la marca tiene menos de cinco menciones recolectadas, When el usuario accede a la sección de sentimiento, Then el sistema muestra el mensaje "Datos insuficientes para el análisis. El análisis estará disponible cuando se recolecten más menciones." | EP04 |
| US13 | Ver menciones por categoría de sentimiento | Como PyME Owner o Agency Manager, quiero filtrar las menciones según su categoría de sentimiento (positivo, negativo, neutro) para priorizar las que requieren atención. | **Escenario 1 – Filtro por sentimiento negativo:** Given que el usuario selecciona "Negativo" en el filtro de sentimiento, When el sistema aplica el filtro, Then el listado muestra únicamente menciones clasificadas como negativas, ordenadas de más reciente a más antigua. **Escenario 2 – Cambio de categoría:** Given que el usuario tiene activo el filtro de sentimiento negativo, When selecciona la categoría "Positivo", Then el sistema actualiza el listado mostrando solo menciones positivas sin necesidad de recargar la página. **Escenario 3 – Sin menciones en la categoría:** Given que la marca no tiene menciones clasificadas en la categoría seleccionada, When el usuario aplica el filtro, Then el sistema muestra el mensaje "No hay menciones en esta categoría." | EP04 |
| US14 | Ver tendencia de sentimiento en el tiempo | Como Agency Manager, quiero ver la evolución del sentimiento de una marca a lo largo del tiempo para evaluar el impacto de las acciones tomadas. | **Escenario 1 – Tendencia con datos históricos:** Given que la marca tiene menciones clasificadas en múltiples fechas, When el usuario accede al gráfico de tendencia, Then el sistema muestra la curva de evolución del sentimiento por semana o mes con opción de ajustar el rango de fechas. **Escenario 2 – Rango sin datos:** Given que el usuario selecciona un rango de fechas en el que no hay menciones registradas, When el sistema intenta generar el gráfico, Then el sistema muestra el mensaje "No hay datos de sentimiento para el período seleccionado." **Escenario 3 – Comparación entre períodos:** Given que el usuario selecciona dos períodos distintos para comparar, When el sistema procesa la solicitud, Then el gráfico muestra ambas curvas diferenciadas por color con un resumen comparativo de la variación porcentual. | EP04 |
| EP05 | Gestión de Alertas | Agrupa la generación de alertas automáticas ante eventos críticos de reputación, notificación al usuario y flujo de respuesta y registro. | — | — |
| US15 | Recibir alertas de menciones críticas | Como PyME Owner o Agency Manager, quiero recibir alertas cuando se detecten menciones negativas críticas para poder actuar de forma inmediata. | **Escenario 1 – Alerta generada y notificada:** Given que el sistema detecta una mención clasificada como negativa y con alto impacto para una marca monitorizada, When se genera la alerta, Then el sistema envía una notificación dentro de la plataforma y por correo electrónico al responsable de la marca. **Escenario 2 – Alerta ya revisada:** Given que el usuario ya revisó y marcó como atendida una alerta, When vuelve a acceder al panel de alertas, Then la alerta aparece en la sección "Atendidas" y no genera notificaciones adicionales. **Escenario 3 – Sin menciones críticas:** Given que no se han detectado menciones críticas en las últimas 24 horas, When el usuario accede al panel de alertas, Then el sistema muestra el mensaje "Sin alertas críticas recientes." | EP05 |
| US16 | Configurar criterios de alerta | Como Agency Manager, quiero configurar los criterios bajo los cuales se generan alertas (umbral de sentimiento, palabras clave críticas) para personalizar el monitoreo según cada cliente. | **Escenario 1 – Configuración guardada:** Given que el Agency Manager define palabras clave críticas y un umbral mínimo de puntuación negativa para la alerta, When guarda la configuración, Then el sistema aplica los nuevos criterios al monitoreo de esa marca y confirma con el mensaje "Criterios de alerta actualizados." **Escenario 2 – Sin criterios definidos:** Given que el usuario intenta activar alertas sin haber definido ningún criterio, When intenta guardar, Then el sistema muestra el mensaje "Debes definir al menos un criterio para activar las alertas." **Escenario 3 – Criterio duplicado:** Given que el usuario intenta agregar una palabra clave crítica que ya existe en la configuración, When confirma la adición, Then el sistema muestra el mensaje "Esta palabra clave ya está configurada como criterio de alerta" y no genera duplicados. | EP05 |
| US17 | Responder a una mención desde la alerta | Como PyME Owner o Agency Manager, quiero poder responder a una mención directamente desde el detalle de una alerta para gestionar rápidamente situaciones críticas de reputación. | **Escenario 1 – Respuesta registrada exitosamente:** Given que el usuario accede al detalle de una alerta y redacta una respuesta en el campo habilitado, When confirma el envío, Then el sistema registra la acción con fecha, hora y contenido de la respuesta, y actualiza el estado de la alerta a "Respondida." **Escenario 2 – Respuesta vacía:** Given que el usuario intenta confirmar el envío sin haber redactado ningún texto, When confirma el envío, Then el sistema muestra el mensaje "El campo de respuesta no puede estar vacío" y no registra la acción. **Escenario 3 – Mención ya respondida:** Given que la mención asociada a la alerta ya tiene una respuesta registrada, When otro usuario de la misma agencia intenta responderla nuevamente, Then el sistema muestra el aviso "Esta mención ya fue respondida" y muestra el historial de respuestas previas. | EP05 |
| EP06 | Reportes y Dashboard | Agrupa la visualización del dashboard de reputación, generación de reportes y exportación de datos. | — | — |
| US18 | Ver dashboard de reputación | Como PyME Owner o Agency Manager, quiero ver un dashboard con las métricas principales de reputación de mi marca para tener una visión general rápida del estado actual. | **Escenario 1 – Dashboard con datos:** Given que la marca tiene menciones y análisis de sentimiento disponibles, When el usuario accede al dashboard, Then el sistema muestra el volumen de menciones, distribución de sentimiento, alertas activas y tendencia reciente en widgets visuales. **Escenario 2 – Dashboard sin datos:** Given que la marca fue creada recientemente y no tiene menciones recolectadas aún, When el usuario accede al dashboard, Then el sistema muestra los widgets vacíos con el mensaje "Comienza el monitoreo para ver los datos de reputación." **Escenario 3 – Actualización del dashboard:** Given que el usuario está visualizando el dashboard y hay nuevas menciones procesadas, When el usuario hace clic en "Actualizar", Then el sistema refresca los widgets con los datos más recientes sin recargar toda la página. | EP06 |
| US19 | Generar reporte de reputación | Como PyME Owner o Agency Manager, quiero generar un reporte de reputación de un período específico para documentar el desempeño de la marca y presentar resultados. | **Escenario 1 – Reporte generado exitosamente:** Given que el usuario selecciona un rango de fechas con datos disponibles y solicita la generación del reporte, When el sistema procesa la solicitud, Then el reporte se genera con métricas de menciones, sentimiento y alertas del período y queda disponible para su descarga. **Escenario 2 – Sin datos en el período:** Given que el usuario selecciona un rango de fechas sin menciones registradas, When solicita la generación del reporte, Then el sistema muestra el mensaje "No hay datos disponibles para el período seleccionado." **Escenario 3 – Error en la generación:** Given que el sistema experimenta un fallo durante la generación del reporte, When el proceso no puede completarse, Then el sistema muestra el mensaje "No se pudo generar el reporte. Intenta nuevamente." y registra el error internamente. | EP06 |
| US20 | Exportar reporte | Como Agency Manager, quiero exportar el reporte de reputación en formato PDF o CSV para compartirlo con mis clientes o equipo interno. | **Escenario 1 – Exportación en PDF exitosa:** Given que el Agency Manager tiene un reporte generado y selecciona el formato PDF, When confirma la exportación, Then el sistema genera el archivo PDF y lo descarga automáticamente en el dispositivo del usuario. **Escenario 2 – Exportación en CSV exitosa:** Given que el usuario selecciona el formato CSV para la exportación, When confirma la acción, Then el sistema genera el archivo CSV con los datos tabulados del reporte y lo descarga automáticamente. **Escenario 3 – Reporte no disponible para exportar:** Given que el usuario intenta exportar un reporte que aún no terminó de generarse, When hace clic en exportar, Then el sistema muestra el mensaje "El reporte aún está siendo generado. Por favor espera." y no inicia la descarga. | EP06 |
| US21 | Comparar reputación entre períodos | Como Agency Manager, quiero comparar métricas de reputación de un cliente entre dos períodos de tiempo para evaluar la evolución y el impacto de las estrategias aplicadas. | **Escenario 1 – Comparación exitosa:** Given que el Agency Manager selecciona dos períodos distintos con datos disponibles para la misma marca, When solicita la comparación, Then el sistema muestra un cuadro comparativo con las variaciones en menciones, sentimiento y alertas entre ambos períodos. **Escenario 2 – Un período sin datos:** Given que uno de los dos períodos seleccionados no tiene datos registrados, When el sistema intenta generar la comparación, Then el sistema muestra el aviso "El período seleccionado no tiene datos disponibles para la comparación." **Escenario 3 – Períodos iguales:** Given que el usuario selecciona el mismo rango de fechas para ambos períodos de comparación, When intenta confirmar, Then el sistema muestra el mensaje "Los períodos seleccionados deben ser diferentes para realizar la comparación." | EP06 |
| EP07 | Landing Page | Agrupa los contenidos y comportamientos del sitio web estático informativo dirigido a visitantes PyME y de agencia. | — | — |
| US22 | Ver propuesta de valor en la landing page | Como visitante de la landing page, quiero ver claramente el propósito y los beneficios de BrandRadar para entender cómo puede ayudarme antes de registrarme. | **Escenario 1 – Visualización correcta del hero:** Given que el visitante accede a la URL principal de BrandRadar, When la página carga completamente, Then el visitante ve el titular de la propuesta de valor, la descripción del producto y un botón de llamada a la acción visible sin hacer scroll. **Escenario 2 – Visualización en dispositivo móvil:** Given que el visitante accede desde un dispositivo móvil, When la página carga, Then el contenido del hero se adapta responsivamente al ancho de la pantalla sin pérdida de información. **Escenario 3 – Clic en llamada a la acción:** Given que el visitante hace clic en el botón "Comenzar gratis" del hero, When el sistema responde al clic, Then el visitante es redirigido a la página de registro de la aplicación web. | EP07 |
| US23 | Ver sección de funcionalidades para PyMEs | Como visitante del segmento PyME en la landing page, quiero ver las funcionalidades específicas de BrandRadar orientadas a pequeñas y medianas empresas para evaluar si el producto cubre mis necesidades. | **Escenario 1 – Sección visible y completa:** Given que el visitante PyME navega hacia la sección de funcionalidades, When la sección carga, Then el visitante visualiza al menos tres funcionalidades clave (monitoreo, alertas, reportes) con iconos, título y descripción breve. **Escenario 2 – Enlace de contacto disponible:** Given que el visitante revisa las funcionalidades y desea más información, When hace clic en el enlace "Habla con nosotros", Then el sistema lo redirige al formulario de contacto o al correo de soporte configurado. **Escenario 3 – Navegación desde el menú:** Given que el visitante hace clic en el ítem "Funcionalidades" del menú de navegación, When el sistema responde, Then la página realiza scroll suave hasta la sección correspondiente sin recargar la página. | EP07 |
| US24 | Ver sección para agencias en la landing page | Como visitante del segmento agencia en la landing page, quiero ver contenido dirigido a agencias digitales para entender cómo BrandRadar puede optimizar la gestión de múltiples clientes. | **Escenario 1 – Sección para agencias visible:** Given que el visitante de agencia navega hasta la sección orientada a agencias, When la sección carga, Then el visitante visualiza los beneficios específicos para agencias (gestión multi-marca, reportes para clientes, alertas priorizadas) con descripción clara. **Escenario 2 – CTA diferenciado para agencias:** Given que el visitante de agencia visualiza la sección, When interactúa con el botón de llamada a la acción de esa sección, Then el sistema lo redirige al registro con el tipo de cuenta "Agency Manager" preseleccionado. **Escenario 3 – Sección visible en dispositivos móviles:** Given que el visitante de agencia accede desde un smartphone, When la sección de agencias carga, Then el contenido se muestra correctamente adaptado al ancho de la pantalla sin desbordamiento. | EP07 |
| US25 | Ver planes y precios en la landing page | Como visitante de la landing page, quiero ver los planes y precios disponibles de BrandRadar para decidir cuál se adapta mejor a mis necesidades antes de registrarme. | **Escenario 1 – Tabla de precios visible:** Given que el visitante accede a la sección de precios, When la sección carga, Then el visitante visualiza al menos dos planes con sus características incluidas, precio mensual y botón de acción individual. **Escenario 2 – Diferenciación de plan recomendado:** Given que la página muestra múltiples planes, When el visitante visualiza la sección, Then el plan recomendado está visualmente destacado con una etiqueta o borde diferenciador. **Escenario 3 – Clic en botón de plan:** Given que el visitante hace clic en el botón de un plan específico, When el sistema responde al clic, Then el visitante es redirigido al formulario de registro con el plan correspondiente preseleccionado. | EP07 |
| US26 | Enviar mensaje de contacto desde la landing page | Como visitante de la landing page, quiero enviar un mensaje de contacto a BrandRadar para resolver dudas antes de registrarme. | **Escenario 1 – Envío exitoso:** Given que el visitante completa todos los campos del formulario (nombre, correo y mensaje), When hace clic en "Enviar", Then el sistema registra el mensaje, muestra la confirmación "Tu mensaje fue enviado. Te contactaremos pronto." y limpia el formulario. **Escenario 2 – Correo inválido:** Given que el visitante ingresa un formato de correo inválido en el formulario de contacto, When intenta enviar, Then el sistema resalta el campo de correo con el mensaje "Ingresa un correo válido" y no envía el formulario. **Escenario 3 – Campos vacíos:** Given que el visitante intenta enviar el formulario sin completar algún campo obligatorio, When hace clic en "Enviar", Then el sistema resalta los campos vacíos y muestra el mensaje "Completa todos los campos obligatorios." | EP07 |
| EP08 | RESTful API — Autenticación | Endpoints técnicos para registro, verificación y autenticación de usuarios en el sistema. | — | — |
| TS01 | Endpoint de registro de usuario | Como Developer, quiero implementar el endpoint POST /api/v1/auth/register para que la aplicación pueda registrar nuevos usuarios mediante la API. | **Escenario 1 – Registro exitoso (201):** Given que el Developer envía una solicitud POST con nombre, correo, contraseña válidos y tipo de cuenta, When el servidor procesa la solicitud, Then retorna un status 201 Created con el id del usuario creado y un mensaje de confirmación. **Escenario 2 – Correo duplicado (409):** Given que el Developer envía una solicitud POST con un correo ya registrado en el sistema, When el servidor procesa la solicitud, Then retorna un status 409 Conflict con el mensaje "El correo ya está registrado." **Escenario 3 – Datos inválidos (400):** Given que el Developer envía una solicitud POST con campos faltantes o un formato de correo inválido, When el servidor valida la entrada, Then retorna un status 400 Bad Request con los campos que presentan error detallados en el cuerpo de la respuesta. | EP08 |
| TS02 | Endpoint de login | Como Developer, quiero implementar el endpoint POST /api/v1/auth/login para que la aplicación pueda autenticar usuarios y generar tokens de acceso. | **Escenario 1 – Login exitoso (200):** Given que el Developer envía credenciales válidas de un usuario activo, When el servidor autentica la solicitud, Then retorna un status 200 OK con el token de acceso JWT y el tiempo de expiración. **Escenario 2 – Credenciales inválidas (401):** Given que el Developer envía una contraseña incorrecta para un correo registrado, When el servidor valida las credenciales, Then retorna un status 401 Unauthorized con el mensaje "Credenciales inválidas." **Escenario 3 – Cuenta no verificada (403):** Given que el Developer envía credenciales correctas de un usuario cuyo correo no ha sido verificado, When el servidor procesa la solicitud, Then retorna un status 403 Forbidden con el mensaje "La cuenta no ha sido verificada." | EP08 |
| TS03 | Endpoint de verificación de correo | Como Developer, quiero implementar el endpoint GET /api/v1/auth/verify?token={token} para que la aplicación pueda verificar cuentas mediante el enlace enviado por correo. | **Escenario 1 – Verificación exitosa (200):** Given que el Developer realiza una solicitud GET con un token válido y vigente, When el servidor valida el token, Then retorna un status 200 OK, activa la cuenta del usuario y retorna el mensaje "Cuenta verificada exitosamente." **Escenario 2 – Token expirado (400):** Given que el Developer realiza la solicitud con un token cuya vigencia ha expirado, When el servidor intenta validarlo, Then retorna un status 400 Bad Request con el mensaje "El token de verificación ha expirado." **Escenario 3 – Token inválido (404):** Given que el Developer realiza la solicitud con un token que no existe en el sistema, When el servidor busca el token, Then retorna un status 404 Not Found con el mensaje "Token de verificación no encontrado." | EP08 |
| EP09 | RESTful API — Marcas | Endpoints técnicos para creación, configuración y consulta de marcas. | — | — |
| TS04 | Endpoint de creación de marca | Como Developer, quiero implementar el endpoint POST /api/v1/brands para que la aplicación pueda registrar nuevas marcas asociadas a un usuario autenticado. | **Escenario 1 – Creación exitosa (201):** Given que el Developer envía una solicitud POST autenticada con nombre, descripción y keywords válidos, When el servidor procesa la solicitud, Then retorna un status 201 Created con el id y los datos de la marca creada. **Escenario 2 – Sin autenticación (401):** Given que el Developer envía la solicitud sin token de autenticación o con un token inválido, When el servidor valida la autorización, Then retorna un status 401 Unauthorized con el mensaje "Autenticación requerida." **Escenario 3 – Nombre duplicado (409):** Given que el Developer envía el nombre de una marca que ya existe para ese usuario, When el servidor valida la solicitud, Then retorna un status 409 Conflict con el mensaje "Ya existe una marca con ese nombre para este usuario." | EP09 |
| TS05 | Endpoint de consulta de marcas | Como Developer, quiero implementar el endpoint GET /api/v1/brands para que la aplicación pueda obtener la lista de marcas del usuario autenticado. | **Escenario 1 – Lista retornada (200):** Given que el Developer envía una solicitud GET autenticada, When el servidor procesa la solicitud, Then retorna un status 200 OK con el arreglo de marcas del usuario incluyendo id, nombre, estado del monitoreo y cantidad de menciones. **Escenario 2 – Sin marcas registradas (200 vacío):** Given que el usuario autenticado no tiene marcas registradas aún, When el Developer realiza la solicitud, Then el servidor retorna un status 200 OK con un arreglo vacío y el mensaje "No hay marcas registradas." **Escenario 3 – Sin autenticación (401):** Given que el Developer envía la solicitud sin token válido, When el servidor verifica la autenticación, Then retorna un status 401 Unauthorized. | EP09 |
| EP10 | RESTful API — Menciones | Endpoints técnicos para consulta y filtrado de menciones recolectadas por el sistema. | — | — |
| TS06 | Endpoint de consulta de menciones | Como Developer, quiero implementar el endpoint GET /api/v1/brands/{id}/mentions para que la aplicación pueda obtener las menciones de una marca con soporte de filtros por fecha, fuente y sentimiento. | **Escenario 1 – Consulta exitosa con filtros (200):** Given que el Developer envía una solicitud GET autenticada con parámetros de query válidos (dateFrom, dateTo, source, sentiment), When el servidor procesa la consulta, Then retorna un status 200 OK con el listado paginado de menciones que cumplen los criterios. **Escenario 2 – Marca no encontrada (404):** Given que el Developer envía el id de una marca que no pertenece al usuario autenticado o no existe, When el servidor busca la marca, Then retorna un status 404 Not Found con el mensaje "Marca no encontrada." **Escenario 3 – Sin resultados (200 vacío):** Given que el Developer aplica filtros que no coinciden con ninguna mención registrada, When el servidor ejecuta la consulta, Then retorna un status 200 OK con un arreglo vacío. | EP10 |
| EP11 | RESTful API — Sentimiento | Endpoints técnicos para consulta del análisis de sentimiento y tendencias por marca. | — | — |
| TS07 | Endpoint de análisis de sentimiento | Como Developer, quiero implementar el endpoint GET /api/v1/brands/{id}/sentiment para que la aplicación pueda obtener el resumen de sentimiento de una marca en un rango de fechas. | **Escenario 1 – Análisis retornado (200):** Given que el Developer envía una solicitud GET autenticada con un rango de fechas válido para una marca con menciones clasificadas, When el servidor procesa la solicitud, Then retorna un status 200 OK con el porcentaje de menciones positivas, negativas y neutras y el score general. **Escenario 2 – Sin datos de sentimiento (404):** Given que la marca no tiene menciones clasificadas para el rango de fechas indicado, When el servidor ejecuta la consulta, Then retorna un status 404 Not Found con el mensaje "No hay datos de sentimiento para el período indicado." **Escenario 3 – Parámetros de fecha inválidos (400):** Given que el Developer envía un rango de fechas con formato incorrecto o con fecha de inicio mayor a la de fin, When el servidor valida los parámetros, Then retorna un status 400 Bad Request con la descripción del error de validación. | EP11 |
| EP12 | RESTful API — Alertas | Endpoints técnicos para consulta, gestión y respuesta de alertas de reputación. | — | — |
| TS08 | Endpoint de consulta de alertas | Como Developer, quiero implementar el endpoint GET /api/v1/brands/{id}/alerts para que la aplicación pueda obtener las alertas activas y atendidas de una marca. | **Escenario 1 – Alertas retornadas (200):** Given que el Developer envía una solicitud GET autenticada para una marca con alertas registradas, When el servidor procesa la solicitud, Then retorna un status 200 OK con el listado de alertas incluyendo id, tipo, estado, mención asociada y fecha de generación. **Escenario 2 – Sin alertas activas (200 vacío):** Given que la marca no tiene alertas activas o históricas registradas, When el Developer realiza la solicitud, Then el servidor retorna un status 200 OK con un arreglo vacío. **Escenario 3 – Sin autenticación (401):** Given que el Developer realiza la solicitud sin token válido, When el servidor valida la autorización, Then retorna un status 401 Unauthorized. | EP12 |
| TS09 | Endpoint de respuesta a alerta | Como Developer, quiero implementar el endpoint PATCH /api/v1/alerts/{id}/respond para que la aplicación pueda registrar la respuesta a una alerta y actualizar su estado. | **Escenario 1 – Respuesta registrada (200):** Given que el Developer envía una solicitud PATCH con el id de alerta válido y el contenido de la respuesta, When el servidor procesa la solicitud, Then retorna un status 200 OK con la alerta actualizada en estado "Respondida" y los datos de la respuesta registrada. **Escenario 2 – Alerta no encontrada (404):** Given que el Developer envía el id de una alerta que no existe o no pertenece al usuario autenticado, When el servidor busca la alerta, Then retorna un status 404 Not Found con el mensaje "Alerta no encontrada." **Escenario 3 – Respuesta vacía (400):** Given que el Developer envía la solicitud sin el campo de contenido de respuesta o con el campo vacío, When el servidor valida el cuerpo, Then retorna un status 400 Bad Request con el mensaje "El contenido de la respuesta es requerido." | EP12 |
| EP13 | RESTful API — Reportes | Endpoints técnicos para generación y exportación de reportes de reputación. | — | — |
